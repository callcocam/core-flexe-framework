...
class ControllerGenerator extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp('Altere o tempCidadelate para refletir a alteração')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $directory_controller =sprintf("%s/Controller",$module_directory);
        $continua = true;
        ...
        if ($continua) {
            ...
            $template = file_get_contents(sprintf("%s/data/template/Controller", ROOT));

            $template = str_replace($this->aFind, $this->aSub, $template);


            file_put_contents(sprintf("%s/%sController.php", $directory_controller, $this->filteredName($input->getArgument('controller'))), $template);
